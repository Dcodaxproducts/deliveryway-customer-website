...
describe("fetchMenuItemDetailsByIds", () => {
  ...
  it("fetches customer cuisines with branch context and preserves happy-hour summary metadata", async () => {
  ...
  it("fetches customer cuisine items with embedded happy-hour pricing", async () => {
    getItemsMock.mockResolvedValueOnce({
      data: {
        items: [
          {
  ...
  it("fetches customer item detail by slug with embedded variation happy-hour pricing", async () => {
    getItemsMock.mockResolvedValueOnce({
      data: {
        id: "item-1",
        slug: "margherita",
